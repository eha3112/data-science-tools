...
class PC_Analysis:
    ...
    def projection_PC_space(self, PC_a, PC_b, target, quality_ratio_a=None, quality_ratio_b=None):
        ...
        trace_1 = go.Scattergl(x=individuals_PC[target_1, PC_a], y=individuals_PC[target_1, PC_b], mode='markers',
                             marker={'size': 4}, name="y_1", hovertext=individuals_name_[target_1], opacity=0.7)
        trace_0 = go.Scattergl(x=individuals_PC[target_0, PC_a], y=individuals_PC[target_0, PC_b], mode='markers',
    ...
    def projection_PC_space_mdf(self, PC_a, PC_b, target=None, quality_ratio_a=None, quality_ratio_b=None):
        """ Projection of individials in the factor-plane [PC_a, PC_b] with the respected quality of representation
        quality_ratio_a and quality_ratio_b.
        parameters:
        -----------
        ...
        if (target is None) or len(target.unique())==1:
            trace = go.Scattergl(x=individuals_PC[:, PC_a], y=individuals_PC[:, PC_b], mode='markers',
                             marker={'size': 4}, name="y_1", hovertext=individuals_name_, opacity=0.7)
            ...
            for label in target.unique():
                ...
                temp_trace = go.Scattergl(x=individuals_PC[temp_target, PC_a], y=individuals_PC[temp_target, PC_b], mode='markers',
                                  marker={'size': 4}, name=str(label), hovertext=individuals_name_[temp_target], opacity=0.7)
...
if __name__=="__main__":
    # laod data
    path ="../"
    data = pd.read_hdf(path+'store.h5', 'data')
    target = pd.read_hdf(path+'store.h5', 'target')

    var_cat = list(pd.read_hdf(path+'store.h5', 'var_cat'))
    var_num = list(pd.read_hdf(path+'store.h5', 'var_num'))

    # initiate a PCA object
    PCA = PC_Analysis(data.loc[:, var_num])

    projection_12 = PCA.projection_PC_space_mdf(PC_a=1, PC_b=2, target=target, quality_ratio_a=20, quality_ratio_b=20)
    projection_12.show()